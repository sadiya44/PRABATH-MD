...
class komiku {
    static async latest() {
        try {
            const { data } = await Utils_1.Axios.get(Constant_1.KomikuIdBaseUrl, {}).catch((e) => e?.response);
            const $ = (0, Utils_1.Cheerio)(data);
            const _temp = [];
            $(komiku._latestLoader.base).each((i, e) => {
                const title = $(e).find(komiku._latestLoader.titleLoader).text();
                const updated = $(e).find(komiku._latestLoader.updatedLoader).text();
                const chapter = $(e).find(komiku._latestLoader.chapterLoader).text();
                let url = $(e)
                ...
                if (url.startsWith("http")) {
                    ...
                    url = Constant_1.KomikuIdBaseUrl + url;
                }
    ...
    static async detail(url) {
        try {
            if (!url.includes(Constant_1.KomikuIdBaseUrl)) {
            ...
            if (/\/manga\//i.test(url)) {
                ...
                $(komiku._detailLoader.manga.chaptersLoader.base).each((i, e) => {
                    const chapter = $(e)
                        ...
                        .text();
                    const url = Constant_1.KomikuIdBaseUrl +
    ...
    static async search(query) {
        try {
            const { data } = await Utils_1.Axios.get(Constant_1.KomikuIdBaseUrl.replace("https://", "https://api."), {
                params: {
                    post_type: "manga",
                    s: query,
                },
...
komiku._failed = {
    latest: `failed to get latest data from ${Constant_1.KomikuIdBaseUrl}`,
    detail: `failed to get detail data from ${Constant_1.KomikuIdBaseUrl}`,
    search: `failed to get search data from ${Constant_1.KomikuIdBaseUrl}`,
};
...
const detail = async function (url) {
