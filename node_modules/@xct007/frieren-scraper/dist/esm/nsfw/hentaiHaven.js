...
import { hentaiHavenBaseUrl } from "../Constant.js";
        ...
        $("div[role='tabpanel'] > div").each((i, e) => {
            ...
            $(_metadata).each((_i, _e) => {
                const _sum_title = $(_e)
                    ...
                    .trim()
                    .toLowerCase();
                const _sum_content = $(_e).find(".summary-content").text().trim();
                _metadatas.push({
...
export async function detail(url) {
    try {
        ...
        const $ = Cheerio(data);
        const title = $(".post-title > h1").text().trim() || $("#chapter-heading.h4").text().trim();
        if (!title) {
        ...
        $(".listing-chapters_wrap > ul > li").each((i, e) => {
            ...
            const release_date = $(e).find(".chapter-release-date > i").text();
            const url = $(e).find("a").attr("href");
            episodes.push({
                episode,
                release_date,
